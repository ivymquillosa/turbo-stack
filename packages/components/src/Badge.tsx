...
const Badge=React.forwardRef<HTMLDivElement, IBadgeProps>(
    ({variant='soft', color='primary', size, radius, className, label, children }, ref) => {

    return (
        <div ref={ref} className={cm(badgeStyles.variant[variant][color] , badgeVariants({size, radius, className}))}>
            {children || label}
        </div>
    )
    }
